...
# constructor
#-----------------------------------------------------------------------
    def __init__(self, pin_step, pin_dir, pin_en, baudrate=115200, serialport=2, rx=16, tx=17):
        
        self.tmc_uart = TMC_UART(serialport, baudrate, rx, tx)
...
#-----------------------------------------------------------------------
    ...
    async def async_runToPositionSteps(self, steps, movement_abs_rel=None):
        if movement_abs_rel is not None:
            this_movement_abs_rel = movement_abs_rel
        else:
            this_movement_abs_rel = self._movement_abs_rel
        ...
        while self.distanceToGo() != 0 and not self._stop:
            step_made = self.runSpeed()
            if step_made:
                self.computeNewSpeed()
            await uasyncio.sleep_ms(1)  # Sleep to allow asyncio scheduler to run
